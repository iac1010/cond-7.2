export type Client = {
  ...
  locations?: { 
    id: string; 
    name: string; 
    qrType?: 'chat' | 'feedback' | 'external' | 'quick';
    externalUrl?: string;
  }[];
  tower?: string;
  unit?: string;
...
export type Appointment = {
  ...
  type: 'TICKET' | 'MEETING' | 'OTHER' | 'PREVENTIVA' | 'CORRETIVA';
  ticketId?: string;
  notes?: string;
};
...
export type Classified = {
  ...
  description: string;
  price?: number;
  category: 'PRODUTO' | 'SERVICO' | 'CARONA' | 'OUTRO';
  authorId: string;
  authorName: string;
  contactPhone: string;
  createdAt: string;
...
export interface AppState {
  ...
  addClassified: (classified: Omit<Classified, 'id' | 'createdAt' | 'status'>) => void;
  updateClassified: (id: string, classified: Partial<Classified>) => void;
