        ...
        <div className="flex items-start gap-6">
          {companyLogo && (
            <img 
              src={companyLogo} 
              alt="Logo" 
              className="w-20 h-20 object-contain rounded-2xl bg-slate-50 p-2 border border-slate-100" 
      ...
      {client && (
        <div className="bg-slate-50 rounded-3xl p-6 mb-8 border border-slate-100">
          ...
          <div className="grid grid-cols-2 gap-8">
            <div>
              <p className="text-[0.625rem] font-black uppercase tracking-widest text-slate-400 mb-1">Condomínio / Prédio</p>
        ...
        <table className="w-full border-collapse">
          <thead>
            <tr className="bg-slate-900 text-white">
              <th className="text-left py-4 px-6 text-[0.625rem] font-black uppercase tracking-widest rounded-tl-2xl">Item / Produto</th>
              <th className="text-center py-4 px-6 text-[0.625rem] font-black uppercase tracking-widest">Categoria</th>
              <th className="text-right py-4 px-6 text-[0.625rem] font-black uppercase tracking-widest rounded-tr-2xl">Quantidade</th>
            </tr>
          </thead>
          <tbody>
            {items.map((qi, idx) => {
              ...
              return (
                <tr key={qi.supplyItemId} className={idx % 2 === 0 ? 'bg-white' : 'bg-slate-50'}>
                  ...
                  <td className="py-4 px-6 border-b border-slate-100 text-center">
                    <span className="px-3 py-1 bg-slate-200 text-[0.625rem] font-black uppercase tracking-widest rounded-lg text-slate-600">
                      {item?.category || '-'}
                    </span>
                  </td>
      ...
      <div className="mt-auto pt-12 border-t border-slate-200 break-inside-avoid">
        <div className="grid grid-cols-2 gap-12">
          <div className="flex flex-col items-center">
            <p className="text-[0.625rem] font-black uppercase tracking-widest text-slate-400 mb-4 text-center">Assinatura do Responsável</p>
            <div className="border-b border-slate-900 h-12 w-full flex items-center justify-center">
              {companySignature && (
                <img 
          ...
          <div>
            <p className="text-[0.625rem] font-black uppercase tracking-widest text-slate-400 mb-4 text-center">Data de Recebimento</p>
            <div className="border-b border-slate-900 h-12 w-full flex items-end justify-center pb-2 font-bold text-slate-300">
              ____ / ____ / ________
            </div>
          </div>
        </div>
        <p className="text-center text-[0.625rem] text-slate-400 mt-12 font-medium">
