...
export const BiaBrain: React.FC = () => {
  ...
  useEffect(() => {
    if (!isSupabaseConfigured) {
      console.warn('BiaBrain: Supabase não configurado. Bia está offline.');
      return;
    }

    if (isLocalSupabase) {
      console.warn('BiaBrain: Supabase está em localhost. Bia está offline no preview.');
      return;
    }
    ...
    const pollingInterval = setInterval(async () => {
      try {
        ...
        if (error) {
          // Only log if it's not a fetch error to avoid spamming the console
