  ...
  return (
    <div className={`relative bg-slate-900/70 backdrop-blur-[40px] border border-white/20 p-4 md:p-6 rounded-3xl md:rounded-[2.5rem] shadow-2xl overflow-hidden text-white ${className}`}>
        ...
        <div className="flex items-center justify-between mb-4 md:mb-6 relative z-10 gap-2">
          <div className="flex items-center gap-2 md:gap-3 min-w-0">
            ...
            <div className="min-w-0">
              <h3 className="text-[0.625rem] md:text-sm font-black uppercase tracking-[0.1em] md:tracking-[0.2em] text-white/90 truncate">Gestão de OS</h3>
              <p className="text-[0.5rem] md:text-[0.625rem] text-white/40 font-bold uppercase tracking-widest truncate">Monitoramento Live</p>
            </div>
          </div>
          <div className="flex items-center gap-1.5 md:gap-2 bg-white/5 px-2 md:px-3 py-1 md:py-1.5 rounded-full border border-white/10 backdrop-blur-md shrink-0">
            ...
            <span className="text-[0.5rem] md:text-[0.625rem] font-black text-emerald-400 uppercase tracking-wider">Live</span>
          </div>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6 relative z-10">
        ...
        <div className="space-y-6">
          <div className="bg-white/5 rounded-2xl md:rounded-3xl p-3 md:p-4 border border-white/5 backdrop-blur-sm">
            <h4 className="text-[0.5rem] md:text-[0.625rem] font-black uppercase tracking-widest text-white/50 mb-3 md:mb-4 flex items-center gap-2 truncate">
            ...
            <div className="mt-3 md:mt-4 space-y-1.5 md:space-y-2">
              {pieData.map((item) => (
                <div key={item.name} className="flex items-center justify-between text-[0.5625rem] md:text-[0.625rem]">
          ...
          <div className="bg-white/5 rounded-2xl md:rounded-3xl p-3 md:p-4 border border-white/5 backdrop-blur-sm">
            <h4 className="text-[0.5rem] md:text-[0.625rem] font-black uppercase tracking-widest text-white/50 mb-2 md:mb-3 flex items-center gap-2 truncate">
            ...
            <div className="space-y-2 md:space-y-3">
              {towerData.length > 0 ? towerData.map((tower) => (
                <div key={tower.name} className="space-y-1">
                  <div className="flex justify-between text-[0.5rem] md:text-[0.5625rem] font-bold uppercase tracking-tight">
                ...
                <p className="text-[0.5rem] md:text-[0.625rem] text-white/30 italic">Sem dados</p>
              )}
            </div>
          </div>
        </div>
        ...
        <div className="space-y-6">
          <div className="bg-white/5 rounded-3xl p-4 border border-white/5 backdrop-blur-sm">
            <div className="flex justify-between items-start mb-4">
              <h4 className="text-[0.625rem] font-black uppercase tracking-widest text-white/50">Visão Detalhada</h4>
          ...
          <div className="bg-white/5 rounded-3xl p-4 border border-white/5 h-48">
            <h4 className="text-[0.625rem] font-black uppercase tracking-widest text-white/50 mb-4">Soluções de OS (Dias)</h4>
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={lineData}>
                ...
                />
                <RechartsTooltip 
                  contentStyle={{ backgroundColor: '#1e293b', border: 'none', borderRadius: '8px', fontSize: '0.625rem' }}
                  itemStyle={{ color: '#fff' }}
                />
              </LineChart>
            </ResponsiveContainer>
        ...
        <div className="space-y-6">
          <div className="bg-white/5 rounded-3xl p-4 border border-white/5">
            <h4 className="text-[0.625rem] font-black uppercase tracking-widest text-white/50 mb-4">Fluxo de OS Semanal</h4>
              ...
              {realWeeklyFlow.map((item, idx) => (
                ...
                >
                  <div className="flex items-center gap-2 min-w-0 flex-1">
                    <span className="text-[0.4375rem] font-black text-white/30 uppercase w-5 shrink-0">{item.day}</span>
                    <span className="text-[0.5rem] font-bold truncate text-white/80 group-hover/item:text-white transition-colors">{item.task}</span>
                  </div>
                  <span className={`text-[0.375rem] font-black uppercase px-1 py-0.5 rounded-md bg-white/5 shrink-0 ml-1 ${item.color}`}>
                    {item.status}
                  </span>
                </div>
              ))}
              {realWeeklyFlow.length === 0 && (
                <p className="text-[0.5rem] text-white/30 italic text-center py-4">Nenhuma OS recente</p>
              )}
            </div>
          </div>

          <div className="bg-white/5 rounded-3xl p-4 border border-white/5">
            <h4 className="text-[0.625rem] font-black uppercase tracking-widest text-white/50 mb-4">Notificações & Alertas</h4>
            <div className="space-y-3">
              <div className="flex gap-3">
                ...
                <div>
                  <p className="text-[0.625rem] font-bold text-white/90 leading-tight">Baixo Nível de Água (Cisterna C)</p>
                  <p className="text-[0.5rem] text-white/40 uppercase mt-0.5">Há 15 minutos</p>
                </div>
              </div>
              <div className="flex gap-3">
                ...
                <div>
                  <p className="text-[0.625rem] font-bold text-white/90 leading-tight">Entrega de Material de Limpeza (23/03)</p>
                  <p className="text-[0.5rem] text-white/40 uppercase mt-0.5">Agendado</p>
                </div>
              </div>
            </div>
          </div>
      ...
      <div className="mt-6 pt-4 border-t border-white/5 flex justify-end relative z-10">
        <button className="flex items-center gap-2 text-[0.625rem] font-black uppercase tracking-widest text-white/40 hover:text-white transition-colors group">
