    ...
    <div className="w-full h-full bg-slate-900/40 backdrop-blur-2xl p-4 flex flex-col justify-between group relative overflow-hidden border border-white/10 shadow-2xl active:scale-95">
      ...
      <div className="flex items-center justify-between mb-4 relative z-10">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-[0.625rem] font-black uppercase tracking-[0.2em] text-white/90">Reservatório de OS</h3>
            <p className="text-[0.5rem] text-white/40 font-bold uppercase tracking-widest">Últimas Realizadas</p>
          </div>
        </div>
        <Link 
          to={isEditMode ? '#' : "/tickets"} 
          onClick={(e) => isEditMode && e.preventDefault()}
          className="text-[0.5625rem] font-black uppercase tracking-widest text-emerald-400 hover:text-emerald-300 transition-colors"
        >
          Ver Tudo
        </Link>
      </div>
          ...
          >
            <div className="flex items-center gap-3 min-w-0">
              <div className="w-7 h-7 rounded-lg bg-white/5 flex items-center justify-center shrink-0 border border-white/5">
                <span className="text-[0.4375rem] font-black text-white/30 uppercase">
                  {safeFormatDate(ticket.date, { weekday: 'short' }).toUpperCase().replace('.', '')}
                </span>
              </div>
              <p className="text-[0.625rem] font-bold text-white/80 truncate group-hover/item:text-white transition-colors">
                {ticket.title || 'Sem título'}
              </p>
            </div>
            <div className={`px-2 py-0.5 rounded-md text-[0.375rem] font-black uppercase tracking-wider shrink-0 bg-white/5 ${
      ...
      <span className="hidden md:block text-[0.6875rem] font-black uppercase tracking-[0.2em] relative z-10 text-white/70 mt-2">Gestão de OS</span>
    </div>
  );
};
