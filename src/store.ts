...
export const useStore = create<AppState>()(
  persist(
    (set, get) => ({
      ...
      addClassified: (classified) => set((state) => ({
        classifieds: [...state.classifieds, { ...classified, id: uuidv4(), createdAt: new Date().toISOString(), status: 'ACTIVE' }]
      })),
      updateClassified: (id, classified) => set((state) => ({
        classifieds: state.classifieds.map(c => c.id === id ? { ...c, ...classified } : c)
      })),
      deleteClassified: (id) => set((state) => ({
        classifieds: state.classifieds.filter(c => c.id !== id)
      })),

      addLostAndFound: (item) => set((state) => ({
        lostAndFound: [...state.lostAndFound, { ...item, id: uuidv4(), status: 'FOUND' }]
      })),
      updateLostAndFound: (id, item) => set((state) => ({
        lostAndFound: state.lostAndFound.map(i => i.id === id ? { ...i, ...item } : i)
      })),
