...
import { Save, X, ClipboardList, Info, Wrench, ShieldAlert, Clock, CheckCircle2, AlertCircle, HelpCircle, Camera, Image as ImageIcon, Trash2, Calendar } from 'lucide-react';
...
export default function TicketForm() {
  ...
  const { id } = useParams();
  const { clients, checklistItems, addTicket, updateTicket, tickets, addAppointment, updateAppointment, appointments } = useStore();
  ...
  useEffect(() => {
    if (id) {
      ...
      if (ticket) {
        ...
        const hasApp = appointments.some(a => a.ticketId === id);
        setAddToCalendar(hasApp);
      }
    }
  }, [id, tickets, appointments]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (id) {
      updateTicket(id, ticketData as any);
      if (addToCalendar) {
        const existingApp = appointments.find(a => a.ticketId === id);
        const appData = {
          title: `OS: ${title || osNumber}`,
          start: date,
          end: date,
          type: (type === 'PREVENTIVA' ? 'PREVENTIVA' : 'CORRETIVA') as any,
          ticketId: id,
          notes: observations
        };
        ...
        if (!existingApp) {
          ...
          updateAppointment(existingApp.id, appData);
        }
      }
    } else {
      const newId = await addTicket(ticketData as any);
      ...
      <div className="max-w-5xl mx-auto w-full relative z-10 mb-6 md:mb-8">
        <div className="flex flex-col sm:flex-row bg-white/5 backdrop-blur-md rounded-2xl p-2 border border-white/10 gap-1 sm:gap-0">
          <button 
            ...
            className={`flex-1 py-3 md:py-4 rounded-xl font-bold tracking-widest uppercase text-[0.625rem] md:text-xs transition-all ${activeTab === 'geral' ? 'bg-white/20 text-white shadow-lg' : 'text-white/40 hover:text-white/60'}`}
          >
            Informações Gerais
          </button>
          <button 
            ...
            className={`flex-1 py-3 md:py-4 rounded-xl font-bold tracking-widest uppercase text-[0.625rem] md:text-xs transition-all ${activeTab === 'servico' ? 'bg-white/20 text-white shadow-lg' : 'text-white/40 hover:text-white/60'}`}
          >
            Detalhes do Serviço
          </button>
          <button 
            ...
            className={`flex-1 py-3 md:py-4 rounded-xl font-bold tracking-widest uppercase text-[0.625rem] md:text-xs transition-all ${activeTab === 'fotos' ? 'bg-white/20 text-white shadow-lg' : 'text-white/40 hover:text-white/60'}`}
          >
            Fotos do Serviço
          </button>
        </div>
              ...
              >
                ...
                <div className="bg-white/5 backdrop-blur-md rounded-[1.5rem] md:rounded-[2.5rem] border border-white/10 p-6 md:p-10 shadow-2xl">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-10">
                    ...
                    <div className="space-y-2 md:space-y-3">
                      <label className="block text-xs md:text-sm font-bold uppercase tracking-widest text-white/40 ml-1">Agenda</label>
                      <button
                        type="button"
                        onClick={() => setAddToCalendar(!addToCalendar)}
                        className={`w-full flex items-center justify-between px-4 md:px-6 py-3 md:py-5 rounded-xl md:rounded-2xl border transition-all ${
                          addToCalendar 
                      ...
                      >
                        <div className="flex items-center gap-3">
                          ...
                          <span className="text-sm md:text-base font-bold uppercase tracking-widest">
                            {addToCalendar ? 'Adicionado à Agenda' : 'Adicionar à Agenda'}
                          </span>
                        </div>
                        <div className={`w-10 h-5 rounded-full relative transition-colors ${addToCalendar ? 'bg-emerald-500' : 'bg-white/10'}`}>
                          <div className={`absolute top-1 w-3 h-3 rounded-full bg-white transition-all ${addToCalendar ? 'left-6' : 'left-1'}`} />
                        </div>
                      </button>
                    </div>
                    ...
                    <div className="md:col-span-2 space-y-2 md:space-y-3">
                      ...
                      <div className="flex flex-wrap gap-2 md:gap-4 p-3 md:p-4 bg-white/5 rounded-xl md:rounded-2xl border border-white/10">
                        ...
                        <div className="flex items-center gap-2 md:gap-3 ml-auto">
                          <span className="text-[0.625rem] md:text-xs font-bold uppercase tracking-widest text-white/40">Personalizada:</span>
                          <input 
                            type="color"
