      ...
      <header className="relative z-10 mb-12 flex flex-col md:flex-row md:items-end justify-between gap-8 border-b border-white/10 pb-8">
        <div className="flex items-center gap-6">
          ...
          <div>
            <div className="flex items-center gap-3 mb-2">
              ...
              <span className="text-[0.625rem] font-black uppercase tracking-[0.4em] text-purple-400/60">AI Financial Intelligence</span>
            </div>
        ...
        <div className="flex items-center gap-3">
          <nav className="flex bg-white/5 p-1 rounded-2xl backdrop-blur-md border border-white/10">
            <button 
              onClick={() => navigate('/')}
              className="px-6 py-3 rounded-xl text-white/40 hover:text-white transition-all flex items-center gap-2 text-[0.625rem] font-black uppercase tracking-widest"
            >
            ...
            <button 
              className="px-6 py-3 rounded-xl bg-white/10 text-white shadow-lg transition-all flex items-center gap-2 text-[0.625rem] font-black uppercase tracking-widest"
            >
          ...
          <motion.button 
            ...
            className="bg-purple-500/10 hover:bg-purple-500/20 text-purple-400 px-8 py-4 flex items-center gap-3 border border-purple-500/30 transition-all rounded-2xl backdrop-blur-2xl font-black uppercase tracking-widest text-[0.625rem] shadow-[0_0_30px_rgba(168,85,247,0.1)] disabled:opacity-50"
          >
        ...
        <div className="flex-1 flex items-center justify-center relative z-10">
          <GlassCard className="max-w-xl text-center p-12">
            ...
            <motion.button
              ...
              whileTap={{ scale: 0.95 }}
              onClick={handleGenerateForecast}
              className="px-10 py-4 bg-purple-600 text-white rounded-2xl font-black uppercase tracking-widest text-[0.625rem] hover:bg-purple-500 transition-all shadow-lg shadow-purple-600/20"
            >
              Iniciar Análise IA
            </motion.button>
          </GlassCard>
              ...
              <div className="mt-8 space-y-4">
                <div className="p-4 bg-white/5 rounded-2xl border border-white/5 flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-[0.625rem] font-bold text-white/50 uppercase tracking-wider">Total Projetado</span>
                  </div>
                ...
                <div className="p-4 bg-white/5 rounded-2xl border border-white/5 flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-[0.625rem] font-bold text-white/50 uppercase tracking-wider">Período</span>
                  </div>
              ...
              <div className="space-y-4">
                {selectedForecast.insights.map((insight, index) => (
                  ...
                  >
                    ...
                    <p className="text-[0.6875rem] text-white/60 leading-relaxed font-medium group-hover:text-white transition-colors">
                      {insight}
                    </p>
                  </motion.div>
                ))}
          ...
          <div className="lg:col-span-8 space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
                ...
                <div className="space-y-3 mt-2">
                  {[
                    ...
                    <div key={idx} className="flex items-center justify-between p-2 bg-white/5 rounded-xl border border-white/5 hover:bg-white/10 transition-all group">
                      <div className="flex items-center gap-3">
                        <span className="text-[0.625rem] font-black text-white/30 uppercase w-6">{item.day}</span>
                        <span className="text-[0.6875rem] text-white/70 font-medium truncate max-w-[100px]">{item.title}</span>
                      </div>
                      <span className={`text-[0.5rem] font-black uppercase px-2 py-1 rounded-md ${
              ...
              <GlassCard title="Obras por Torre (Gasto vs. Orçado)" icon={Building2} className="md:col-span-4">
                ...
                <div className="mt-4 flex justify-between text-[0.5rem] font-black text-white/30 uppercase tracking-widest">
                ...
                <div className="mt-4 grid grid-cols-2 gap-2">
                  {selectedForecast.categories.slice(0, 4).map((cat, index) => (
                    <div key={cat.name} className="flex items-center gap-2">
                      ...
                      <span className="text-[0.5rem] font-bold text-white/40 uppercase tracking-wider truncate">{cat.name}</span>
                    </div>
                  ))}
                </div>
              </GlassCard>
              ...
              <GlassCard title="Filtros & Modelos" icon={Filter} className="md:col-span-4">
                <div className="space-y-4 mt-2">
                  <div className="flex items-center gap-4 p-3 bg-white/5 rounded-2xl border border-white/5 hover:bg-white/10 transition-all cursor-pointer group">
                    ...
                    <div>
                      <div className="text-[0.625rem] font-black text-white/70 uppercase tracking-widest">Filtro: Vencidos</div>
                      <div className="text-[0.5rem] text-white/30 uppercase font-bold">Ativos no Sistema</div>
                    </div>
                  </div>
                  <div className="flex items-center gap-4 p-3 bg-white/5 rounded-2xl border border-white/5 hover:bg-white/10 transition-all cursor-pointer group">
                    ...
                    <div>
                      <div className="text-[0.625rem] font-black text-white/70 uppercase tracking-widest">Modelo: Aditivo</div>
                      <div className="text-[0.5rem] text-white/30 uppercase font-bold">Contrato de Prestação</div>
                    </div>
                  </div>
                  <div className="flex items-center gap-4 p-3 bg-white/5 rounded-2xl border border-white/5 hover:bg-white/10 transition-all cursor-pointer group">
                    ...
                    <div>
                      <div className="text-[0.625rem] font-black text-white/70 uppercase tracking-widest">Modelo: Prestação</div>
                      <div className="text-[0.5rem] text-white/30 uppercase font-bold">Serviço de Terceiros</div>
                    </div>
                  </div>
                </div>
              </GlassCard>
            ...
            <GlassCard title="Alertas de Risco & Notificações" icon={Bell}>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="flex items-center gap-4 p-4 bg-rose-500/5 border border-rose-500/20 rounded-2xl group hover:bg-rose-500/10 transition-all">
                  ...
                  <div>
                    <div className="text-[0.625rem] font-black text-rose-400 uppercase tracking-widest">Alerta Crítico</div>
                    <div className="text-[0.6875rem] text-white/60 font-medium">Possível estouro de orçamento em Manutenção (Torre B)</div>
                  </div>
                </div>
                <div className="flex items-center gap-4 p-4 bg-amber-500/5 border border-amber-500/20 rounded-2xl group hover:bg-amber-500/10 transition-all">
                  ...
                  <div>
                    <div className="text-[0.625rem] font-black text-amber-400 uppercase tracking-widest">Tendência de Alta</div>
                    <div className="text-[0.6875rem] text-white/60 font-medium">Aumento projetado de 12% nos custos de energia</div>
                  </div>
                </div>
              </div>
            </GlassCard>
