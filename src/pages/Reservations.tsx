          ...
          return (
            ...
            >
              <div className="flex justify-between items-start mb-6">
                <div className={`px-3 py-1 rounded-full text-[0.625rem] font-black uppercase tracking-widest border ${getStatusColor(res.status)}`}>
              ...
              <div className="space-y-3 mb-6">
                <div className="flex items-center gap-3 text-white/60">
                  ...
                  {client?.unit && (
                    <span className="bg-white/10 px-2 py-0.5 rounded text-[0.625rem] text-white">Apto {client.unit}</span>
                  )}
                </div>
                <div className="flex items-center gap-3 text-white/60">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="md:col-span-2">
              <label className="block text-xs font-black uppercase tracking-widest text-zinc-400 dark:text-white/40 mb-2">Morador *</label>
              <select 
                required
            ...
            <div className="md:col-span-2">
              <label className="block text-xs font-black uppercase tracking-widest text-zinc-400 dark:text-white/40 mb-2">Área Comum *</label>
              <select 
                required
              ...
              >
                {AREAS.map(area => (
                  <option key={area} value={area} className="bg-white dark:bg-zinc-900 text-zinc-900 dark:text-white">{area}</option>
                ))}
              </select>
            </div>

            <div>
              <label className="block text-xs font-black uppercase tracking-widest text-zinc-400 dark:text-white/40 mb-2">Data *</label>
              <input 
                required
                type="date"
                value={formData.date}
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-xs font-black uppercase tracking-widest text-zinc-400 dark:text-white/40 mb-2">Início *</label>
                <input 
                  required
                  type="time"
              ...
              <div>
                <label className="block text-xs font-black uppercase tracking-widest text-zinc-400 dark:text-white/40 mb-2">Fim *</label>
                <input 
                  required
                  type="time"
            ...
            <div className="md:col-span-2">
              <label className="block text-xs font-black uppercase tracking-widest text-zinc-400 dark:text-white/40 mb-2">Observações</label>
              <textarea 
                value={formData.notes}
                onChange={e => setFormData({...formData, notes: e.target.value})}
                className="w-full bg-zinc-50 dark:bg-white/5 border border-zinc-200 dark:border-white/10 rounded-xl px-4 py-3 outline-none min-h-[100px] resize-none focus:border-zinc-400 dark:focus:border-white/30 transition-all text-zinc-900 dark:text-white"
          ...
          <div className="pt-6 flex justify-end gap-3">
            <button 
              type="button"
              onClick={closeModal}
              className="px-6 py-3 text-zinc-400 dark:text-white/40 hover:text-zinc-900 dark:hover:text-white transition-colors font-bold uppercase tracking-widest text-xs"
            >
              Cancelar
            </button>
            <button 
              type="submit"
              className="bg-zinc-900 dark:bg-white text-white dark:text-black px-10 py-3 rounded-xl font-black uppercase tracking-widest text-xs transition-all active:scale-95"
            >
              Confirmar Reserva
            </button>
          </div>
