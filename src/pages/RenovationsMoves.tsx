          ...
          <div className="lg:col-span-4 space-y-6">
            <GlassCard title="SITUAÇÃO GERAL (Obras & Mudanças)" icon={TrendingUp}>
              <div className="flex flex-col items-center gap-6 py-4">
                <div className="text-[0.625rem] font-bold text-white/50 uppercase tracking-[0.2em]">TOTAL DE TAREFAS: <span className="text-emerald-400">95%</span></div>
                <CircularProgress 
                  value={95} 
                ...
                <div className="w-full space-y-2 mt-4">
                  <div className="flex items-center justify-between text-[0.625rem] font-bold uppercase">
                ...
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={towerData}>
                    ...
                    <XAxis 
                      ...
                      axisLine={false} 
                      tickLine={false} 
                      tick={{ fill: '#ffffff40', fontSize: '0.625rem' }} 
                    />
              ...
              <div className="flex justify-between mt-4">
                {towerData.map((t, i) => (
                  <div key={i} className="text-center">
                    <div className="text-[0.625rem] font-bold text-white/40 uppercase">{t.name}</div>
            ...
            <GlassCard title="ESTATÍSTICAS & FLUXOS GERAIS" icon={TrendingUp}>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="space-y-4">
                  <div className="text-[0.625rem] font-bold text-white/50 uppercase tracking-[0.2em]">CRONOGRAMA DE ATIVIDADES (DIAS)</div>
                ...
                <div className="space-y-4">
                  <div className="text-[0.625rem] font-bold text-white/50 uppercase tracking-[0.2em]">AGENDA DE MUDANÇAS (SEMANAL)</div>
                    ...
                    {[
                      ...
                      <div key={i} className="flex items-center justify-between p-2 rounded-xl bg-white/5 border border-white/5">
                        <div className="flex items-center gap-3">
                          <span className="text-[0.625rem] font-bold text-white/40 uppercase w-8">{item.day}</span>
                  ...
                  <div className="flex items-center gap-2 text-amber-400">
                    ...
                    <span className="text-[0.625rem] font-bold uppercase tracking-wider">TAREFAS CRÍTICAS: R$ 4.500 (1 OS em atraso)</span>
                  </div>
                </div>
              </div>
            </GlassCard>
                ...
                <div className="space-y-4">
                  {[
                    ...
                    <div key={i} className="flex gap-4 p-3 rounded-2xl bg-white/5 border border-white/5">
                      ...
                      <div>
                        <div className={`text-[0.625rem] font-bold uppercase tracking-widest ${
                      ...
                      <div key={renovation.id} className="group flex flex-col md:flex-row justify-between items-start md:items-center p-6 rounded-3xl bg-white/5 border border-white/5 hover:bg-white/10 hover:border-white/10 transition-all gap-4">
                        ...
                        <div className="flex items-center gap-6 w-full md:w-auto justify-between md:justify-end">
                          <div className="text-right">
                            ...
                            <div className="text-[0.5rem] text-white/20 uppercase font-black tracking-widest mt-1">Status da Obra</div>
                          </div>
                      ...
                      <div key={move.id} className="group flex flex-col md:flex-row justify-between items-start md:items-center p-6 rounded-3xl bg-white/5 border border-white/5 hover:bg-white/10 hover:border-white/10 transition-all gap-4">
                        ...
                        <div className="flex items-center gap-6 w-full md:w-auto justify-between md:justify-end">
                          <div className="text-right">
                            ...
                            <div className="text-[0.5rem] text-white/20 uppercase font-black tracking-widest mt-1">Status do Agendamento</div>
                          </div>
        ...
        <div className="space-y-4">
          {activeTab === 'RENOVATIONS' ? (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-2">
                <label className="text-xs font-bold text-zinc-500 dark:text-white/50 uppercase tracking-widest">Morador / Unidade</label>
                <select
                  value={newRenovation.clientId}
                  onChange={(e) => setNewRenovation({ ...newRenovation, clientId: e.target.value })}
                  className="w-full px-4 py-2 rounded-xl border border-zinc-200 dark:border-white/10 bg-zinc-50 dark:bg-white/5 text-zinc-900 dark:text-white outline-none focus:ring-2 focus:ring-blue-500/50"
                >
                  <option value="" className="bg-white dark:bg-zinc-900 text-zinc-900 dark:text-white">Selecione um morador</option>
                  {clients.map(c => <option key={c.id} value={c.id} className="bg-white dark:bg-zinc-900 text-zinc-900 dark:text-white">{c.name} - Apto {c.unit}</option>)}
                </select>
              </div>
              <div className="space-y-2">
                <label className="text-xs font-bold text-zinc-500 dark:text-white/50 uppercase tracking-widest">Título da Obra</label>
                <input
                  type="text"
              ...
              <div className="space-y-2">
                <label className="text-xs font-bold text-zinc-500 dark:text-white/50 uppercase tracking-widest">Data Início</label>
                <input
                  type="date"
              ...
              <div className="space-y-2">
                <label className="text-xs font-bold text-zinc-500 dark:text-white/50 uppercase tracking-widest">Responsável Técnico (ART)</label>
                <input
                  type="text"
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="space-y-2">
                <label className="text-xs font-bold text-zinc-500 dark:text-white/50 uppercase tracking-widest">Tipo</label>
                <select
                  value={newMove.type}
                  onChange={(e) => setNewMove({ ...newMove, type: e.target.value as 'IN' | 'OUT' })}
                  className="w-full px-4 py-2 rounded-xl border border-zinc-200 dark:border-white/10 bg-zinc-50 dark:bg-white/5 text-zinc-900 dark:text-white outline-none focus:ring-2 focus:ring-blue-500/50"
                >
                  <option value="IN" className="bg-white dark:bg-zinc-900 text-zinc-900 dark:text-white">Entrada (Move-in)</option>
                  <option value="OUT" className="bg-white dark:bg-zinc-900 text-zinc-900 dark:text-white">Saída (Move-out)</option>
                </select>
              </div>
              <div className="space-y-2">
                <label className="text-xs font-bold text-zinc-500 dark:text-white/50 uppercase tracking-widest">Data da Mudança</label>
                <input
                  type="date"
                  value={newMove.date}
          ...
          <div className="flex justify-end gap-3 pt-4">
            <button onClick={() => setIsAdding(false)} className="px-6 py-2 text-zinc-400 dark:text-white/40 hover:text-zinc-600 dark:hover:text-white transition-colors font-bold uppercase text-xs tracking-widest">Cancelar</button>
